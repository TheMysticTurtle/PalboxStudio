...
## Generated reference dataset — `ui/static/data/` (the filter/display tables)
Static game data, generated by **`scripts/gen_species.py`** from the PSP dumps + en-GB names.
**Never hand-edit** — re-run `python scripts/gen_species.py` on a game patch. Each file carries
a `_meta` provenance block; the generator validates referential integrity (0 dangling move
codes, 0 unknown elements). The UI loads these and **filters in-memory** (no DB). Mutable user
data (groups/tags/presets) will be a separate SQLite store later.

- **`species.json`** — **406** box-storable pals (`is_pal` only; humans/NPCs + `Quest_` dupes
  excluded). Category split: Natural 226 / Unobtainable 157 / TowerBoss 23. Columns: `code`,
  `name`, `elements`, `category`, `rarity`, `size`, `genus`, `nocturnal`, `alpha`, `deckIndex`,
  `combiRank` (breeding), `captureRate`, `price`, `foodAmount`, `maxStomach`, `maleProbability`,
  `runSpeed`, `rideSpeed`, `scaling{hp,attack,defense}`, `work{13}`, `moves[]` (learnset codes).
  **GAP stubs (not in the dumps — need a wiki scrape): `partnerSkill`, `farmDrops`.**
- **`moves.json`** — 351 move code → `{name, element, power, category}`. Species learnset codes
  join here; 27 boss-only uniques were synthesized so nothing dangles.
- **`passives.json`** — 420 passive code → `{name, rating −3..5, description}`.
- **`elements.json`** — 9 element → `{color}`.
- **`schema.json`** — self-describing column metadata for `species`
  (`type`/`filterable`/`displayable`) → drives the UI filters + result columns.
