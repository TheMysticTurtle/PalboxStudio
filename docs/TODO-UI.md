...
- [x] Passive presets: apply + create/edit/delete named sets up to the reference DB's current
      passive limit through the shared filtered picker. Full-build presets remain a possible later extension.
...
## 4. Drawers / actions
- [x] Advanced: bind IV / per-stat souls / condensation to real save fields through the engine.
...
- [x] Open-file: hand the picked path to the engine to actually load the box.

## 5. Engine + data (parallel track)
- [x] Rust core: load/write `GlobalPalStorage.sav`; compute displayed stats from
